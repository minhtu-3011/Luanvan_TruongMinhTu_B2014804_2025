(function ($) {
    ...
    HT.setupVariantUrl = (res, attribute_id) => {
        ...
        if (
            !productCanonical ||
            productCanonical.trim() === "" ||
            productCanonical.endsWith("/.html")
        ) {
            productCanonical = window.location.href.split("?")[0];
        }
    ...
    HT.setupVariantName = (res) => {
        const titleEl = $(".product-main-title span");
        if (!titleEl.data("original-name")) {
            titleEl.data("original-name", titleEl.text().trim());
        }
        const productName = titleEl.data("original-name");
        const variantName = res?.variant?.languages?.[0]?.pivot?.name || "";
        titleEl.html(
