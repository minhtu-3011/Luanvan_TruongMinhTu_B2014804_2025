(function ($) {
    ...
    // ck3
    HT.addSlide = () => {
        // Lắng nghe sự kiện click nút thêm slide
        $(document).on("click", ".addSlide", function (e) {
            e.preventDefault();

            // Kiểu resource mặc định (Images)
            const type = $(this).data("type") || "Images";

            // Mở CKFinder popup
            CKFinder.popup({
                basePath: "/backend/plugin/ckfinder/", // thêm nếu CKFinder không ở gốc
                resourceType: type,
                chooseFiles: true,
                onInit: function (finder) {
                    finder.on("files:choose", function (evt) {
                        ...
                        // Lặp qua danh sách file chọn
                        evt.data.files.forEach(function (file) {
                            const fileUrl = file.getUrl();
                            html += HT.renderSlideItemHtml(fileUrl);
                        });

                        // Append vào danh sách slide
                        $(".slide-list").append(html);

                        // Kiểm tra thông báo (nếu có hàm hỗ trợ)
                        if (typeof HT.checkSlideNotification === "function") {
                            HT.checkSlideNotification();
                        }
                    });
                },
                ...
                onInit: function (finder) {
                    finder.on("files:choose", function (evt) {
                        ...
                        if (file) {
                            ...
                            _this.siblings("img").attr("src", fileUrl);
                            _this.siblings("input").val(fileUrl);
                        }
                    });
                ...
                ui.item
                    ...
                    .each(function () {
                        ...
                        let itemData = {
                            ...
                            image: _this
                                .find('input[name="slide[image][]"]')
                                .val(),
                            name: _this
                                .find('input[name="slide[name][]"]')
                                .val(),
                            alt: _this.find('input[name="slide[alt][]"]').val(),
                            description: _this
                                .find('input[name="slide[description][]"]')
                                .val(),
                            canonical: _this
                                .find('input[name="slide[canonical][]"]')
                                .val(),
                            window: _this
                                .find('input[name="slide[window][]"]')
                                .val(),
                        };
