        ...
        @if(isset($productCatalogues) && is_object($productCatalogues))
            ...
            @foreach($productCatalogues as $productCatalogue)
                <tr>
                    ...
                    <td class="js-switch-{{$productCatalogue->id}}">
                        <input type="checkbox" value="{{$productCatalogue->publish}}" class="js-switch status" data-field = "publish" data-model = "{{$config['model']}}" 
                        {{($productCatalogue->publish == 1)?'checked':'' }} data-modelid = "{{$productCatalogue->id}}"/>
                    </td>
