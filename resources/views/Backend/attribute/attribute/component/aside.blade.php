            ...
            <div class="col-lg-12">
                <div class="form-row">
                    <span class="image img-cover image-target">
                        <img src="{{ old('image', $attribute->image ?? '/backend/img/notfound.jpg') }}" alt="">
                    </span>
                    ...
                    <div class="mb10">
                        <select name="publish" id="publish" class="form-control">
                            @foreach(config('apps.general.publish') as $key => $val)
                                <option value="{{ $key }}" {{ $key == old('publish', $attribute->publish ?? '') ? 'selected' : '' }}>
                                    {{ $val }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb10">
                        <select name="follow" id="follow" class="form-control">
                            @foreach(config('apps.general.follow') as $key => $val)
                                <option value="{{ $key }}" {{ $key == old('follow', $attribute->follow ?? '') ? 'selected' : '' }}>
                                    {{ $val }}
                                </option>
                            @endforeach
                        </select>
