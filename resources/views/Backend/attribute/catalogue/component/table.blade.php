        ...
        @if(isset($attributeCatalogues) && is_object($attributeCatalogues))
            ...
            @foreach($attributeCatalogues as $attributeCatalogue)
                <tr>
                    ...
                    <td class="js-switch-{{$attributeCatalogue->id}}">
                        <input type="checkbox" value="{{$attributeCatalogue->publish}}" class="js-switch status" data-field = "publish" data-model = "{{$config['model']}}" 
                        {{($attributeCatalogue->publish == 1)?'checked':'' }} data-modelid = "{{$attributeCatalogue->id}}"/>
                    </td>
