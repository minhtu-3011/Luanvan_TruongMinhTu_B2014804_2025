        ...
        @if(isset($slides) && is_object($slides))
            ...
            <tr >
                ...
                <td class="text-center js-switch-{{ $slide->id }}"> 
                    <input type="checkbox" value="{{ $slide->publish }}" class="js-switch status " data-field="publish" data-model="{{ $config['model'] }}" {{ ($slide->publish == 1) ? 'checked' : '' }} data-modelId="{{ $slide->id }}" />
                </td>
