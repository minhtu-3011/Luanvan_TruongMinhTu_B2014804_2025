        ...
        @if(isset($sources) && is_object($sources))
            ...
            <tr >
                ...
                <td class="text-center js-switch-{{ $source->id }}"> 
                    <input type="checkbox" value="{{ $source->publish }}" class="js-switch status " data-field="publish" data-model="{{ $config['model'] }}" {{ ($source->publish == 1) ? 'checked' : '' }} data-modelId="{{ $source->id }}" />
                </td>
