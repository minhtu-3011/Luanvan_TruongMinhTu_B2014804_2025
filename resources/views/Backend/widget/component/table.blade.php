        ...
        @if(isset($widgets) && is_object($widgets))
            ...
            <tr >
                ...
                <td class="text-center js-switch-{{ $widget->id }}"> 
                    <input type="checkbox" value="{{ $widget->publish }}" class="js-switch status " data-field="publish" data-model="{{ $config['model'] }}" {{ ($widget->publish == 1) ? 'checked' : '' }} data-modelId="{{ $widget->id }}" />
                </td>
                <td class="text-center"> 
